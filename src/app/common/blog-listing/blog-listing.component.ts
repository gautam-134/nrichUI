...
export class BlogListingComponent implements OnInit {
  ...
  refresh(page: number) {
    this.loader
      .showLoader(
        this.commonService.fetchBlogsListing(page, this.size, this.searchParam)
      )
      .subscribe((res: any) => {
  ...
  search() {
    this.searchSubscription = fromEvent(
      ...
      .subscribe((data: any) => {
        this.loader
          .showLoader(
            this.commonService.fetchBlogsListing(
              0,
              this.size,
              data.target.value
            )
  ...
  onDelete(id: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.loader.showLoader(this.commonService.deleteBlog(id)).subscribe(
          (res) => {
